# run_exp_f3_large_matrix.py
#
# FULL 5-SEED, 2-DATASET, 2-SHOT VERIFICATION OF EXP-F3-LARGE (122,897 PARAMS)
# Evaluates EXP-F3-Large across seeds [1, 7, 21, 42, 123] on CIFAR-FS and MiniImageNet Native.
# Saves checkpoints, computes MD5 hashes, performs reload accuracy verification,
# and generates checkpoints/exp_f3_large_manifest.json.
...
MANIFEST_PATH = os.path.join(CHECKPOINT_DIR, "exp_f3_large_manifest.json")
...
# Large Model CIFAR (122,897 params)
class IRFEExpF3_Large_CIFAR(nn.Module):
    def __init__(self, c1=32, c2=48, sub_c=24, gate_h=32, embed_dim=48, window_frac=0.50, num_heads=4):
        super().__init__()
...
# Large Model Native (122,897 params)
class IRFEExpF3_Large_Native(nn.Module):
    def __init__(self, c1=32, c2=48, sub_c=24, gate_h=32, embed_dim=48, window_frac=0.50, num_heads=4):
        super().__init__()
        self.embed_dim        = embed_dim
        base_centers          = get_exp_p1_base_centers()
...
def run_matrix():
    print("=" * 100)
    print("EXP-F3-LARGE (122,897 PARAMS) — FULL 5-SEED VERIFICATION MATRIX")
    print(f"Timestamp: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S IST')}")
    print("=" * 100)

    mc = IRFEExpF3_Large_CIFAR()
    mn = IRFEExpF3_Large_Native()
    pc_c = sum(p.numel() for p in mc.parameters() if p.requires_grad)
    pc_n = sum(p.numel() for p in mn.parameters() if p.requires_grad)
    assert pc_c == 122897 and pc_n == 122897, f"Parameter mismatch! Got CIFAR={pc_c}, Native={pc_n}"
    print(f"Parameter Audit Confirmed: EXP-F3-Large (CIFAR & Native) = {pc_c:,} trainable parameters.", flush=True)
    ...
    datasets = [
        ("cifar", IRFEExpF3_Large_CIFAR, cifar_train, cifar_test),
        ("mini",  IRFEExpF3_Large_Native, mini_train,  mini_test)
    ]
    ...
    for ds_name, model_cls, ds_train, ds_test in datasets:
        ...
        for shot in [1, 5]:
            ...
            print(f"==================================================================================", flush=True)
            for seed in SEEDS:
                ckpt_filename = f"exp_f3_large_{ds_name}_{shot}shot_seed{seed}.pt"
    ...
    manifest_data = {
        ...
        "model_class": "IRFEExpF3_Large (WindowedAdaptivePatchLocator, window_frac=0.50)",
        "trainable_parameters": 122897,
    ...
    print("\n" + "=" * 100)
    print(f"EXP-F3-LARGE VERIFICATION MATRIX COMPLETE. MANIFEST SAVED TO {MANIFEST_PATH}")
    print(f"All 20 Checkpoints Verified: {all_verif}")
    print("=" * 100)

    # Save summary results JSON for easy reporting
    summary_path = os.path.join(CHECKPOINT_DIR, "exp_f3_large_results_summary.json")
